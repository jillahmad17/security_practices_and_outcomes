...
func (s *RepositoriesService) ListContributorsGraphQL(ctx context.Context, owner string, repository string, commit string, opts *ListContributorsOptions) ([]*Contributor, *Response, error) {
	//first must get date from commit
	// GetCommit()
	u := fmt.Sprintf("repos/%v/%v/commits/%v", owner, repository, commit)
	req, err := s.client.NewRequest("GET", u, nil)
	var commitDateResponse *CommitDateResponse
	resp, err := s.client.Do(ctx, req, &commitDateResponse)
	if err != nil {
		return nil, resp, err
	}
	commitDate := commitDateResponse.Commit.Author.Date.Format(time.RFC3339)
	u = "graphql"
	u, err = addOptions(u, opts)
	if err != nil {
		return nil, nil, err
	...
	jsonData := map[string]string{
		...
		`, owner, repository, commitDate),
	}

	// jsonValue, err := json.Marshal(jsonData)

	req, err = s.client.NewRequest("POST", u, jsonData)
	if err != nil {
		return nil, nil, err
	}

	var graphqlResponse *GraphQLResponse
	resp, err = s.client.Do(ctx, req, &graphqlResponse)
	if err != nil {
		return nil, resp, err
	}

	nodes := graphqlResponse.Data.Repository.DefaultBranchRef.Target.History.Nodes

	hasNext := graphqlResponse.Data.Repository.DefaultBranchRef.Target.History.PageInfo.HasNextPage
	endCursor := graphqlResponse.Data.Repository.DefaultBranchRef.Target.History.PageInfo.EndCursor
	for hasNext {
		jsonData := map[string]string{
			"query": fmt.Sprintf(`
				{
					repository(owner: "%s", name: "%s") {
						...
						defaultBranchRef {
							...
							target {
								... on Commit {
									history(first: 100, after: "%s", until: "%s") {
										totalCount
										nodes {
											oid
											committedDate
			...
			`, owner, repository, endCursor, commitDate),
		}
		...
		var graphqlResponse2 *GraphQLResponse
		resp, err := s.client.Do(ctx, req, &graphqlResponse2)
		if err != nil {
			return nil, resp, err
		}
		hasNext = graphqlResponse2.Data.Repository.DefaultBranchRef.Target.History.PageInfo.HasNextPage
		endCursor = graphqlResponse2.Data.Repository.DefaultBranchRef.Target.History.PageInfo.EndCursor

		// append nodes
		nodes = append(nodes, graphqlResponse2.Data.Repository.DefaultBranchRef.Target.History.Nodes...)
	}

	// use map to get contributtions count per contributor
	contributions := make(map[string]*Contributor)
	for _, authorNode := range nodes {
		login := authorNode.Author.User.Login
		if login == "" {
			continue
		}
		val, exists := contributions[login]
		if exists {
			*val.Contributions += 1
		} else { //if len(contributions) < 30
			c := &Contributor{}
			c.Login = new(string)
			*c.Login = login

			// parse, _ := strconv.ParseInt(authorNode.Author.User.ID, 10, 64)
			c.NodeID = new(string)
			*c.NodeID = authorNode.Author.User.ID
			c.Contributions = new(int)
			*c.Contributions = 1
			contributions[login] = c
		}
	}

	var contributor []*Contributor
	for _, c := range contributions {
		contributor = append(contributor, c)
	}
